...
|------|-----------|---------|
| `dashboard_views_global.json` | `index=_audit provenance="UI:Dashboard:*"` with view_session de-duplication + `\| join` against `/servicesNS/-/-/data/ui/views f=eai:acl` for owner/sharing | Dashboard usage ranked by views per app + owner attribution |
...
|------|-----------|------------|---------|
| `dashboard_views_global.json` | `_audit` with provenance + `/data/ui/views` REST join | `app`, `dashboard_name`, `view_count`, `unique_users`, `viewers`, `last_viewed`, `owner`, `sharing` | Migration prioritization by dashboard usage + Owner attribution for the Splunk Explorer |
